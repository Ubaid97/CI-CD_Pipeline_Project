...
import sys, os
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'src/http_management'))
import http_manager
...
class TestHtmlObjectManager:
    ...
    def html_manager_object(self):
        if get_test_env_setting() == 'live':
            return http_manager.HttpManager(itjobswatch_home_page_url())
        else:
            return http_manager.HttpManager(itjobswatch_home_page_test_file())
    ...
    def test_incorrect_file_or_url_path_raises_name_error(self):
        with pytest.raises(NameError):
            assert http_manager.HttpManager("test")
